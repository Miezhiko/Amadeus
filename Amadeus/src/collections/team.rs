...
fn get_discord_servers() -> Discords {
  ...
  for disc in SERVERS.iter() {
    let discord = DiscordFields
                    ...
                    , events:   disc.events.map( ChannelId::new )
                    , log:      disc.log.map( ChannelId::new )
                    };
